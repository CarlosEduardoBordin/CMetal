...
class VerificationProcess:
    def __init__(self, parent_mdi : wx.MDIParentFrame, linear_mass_text, d_text, bf_text, tw_text, tf_text, h_text, d_l_text, area_text, i_x_text,
                 ...
                 flb, fnt, fnc, fcx, fcy, mfx, mfy, y_um, g, e, cb, perfil_name, type, frame_name, save_path):
    ...
    # compressao
            ...
            if 0.35 <= kc <= 0.76:
                ...
                kc_passou = False
                texto_kc = (r"\textcolor{red}{Verificação de esbeltez local kc não atendida ,kc deve estar dentro do intervalo  de 0.35 a 0.76!}")
        ...
        memoria_calculo_shear_x = {
            ...
            "corpo": [
                ...
                {"tipo": "paragrafo", "conteudo": "\n Área efetiva do cisalhamento: \n "},
                {
                    "tipo": "formula",
                    "conteudo": (
                            r"A_w = 2 * d * t_w \Rightarrow 2 * " + f"{self.d_text.magnitude:.{self.casa_decimal_comprimento}f} * {self.tw_text.magnitude:.{self.casa_decimal_comprimento}f} = {aw:.2f}"
                    )
                },
            ...
            "corpo": [
                ...
                },
                {
                    ...
                    "conteudo": (
                            ...
                            f"{self.e.magnitude:.{self.casa_decimal_pressao}f} * {self.i_t_text.magnitude:.{self.casa_decimal_momento}f}" + r"} = " +
                            f"{beta:.2f}"
                    )
                },
                {
        ...
        memoria_calculo_moment_x = {
            ...
            "corpo": [
                ...
                },
                {
                    "tipo": "formula",
                    "conteudo": (
                        rf"\lambda_r = 0.95 * \sqrt{{\frac{{E}}{{f_y}}}} \Rightarrow 0.38 * \sqrt{{\frac{{{self.e.magnitude:.{self.casa_decimal_pressao}f}}}{{{self.fy.magnitude:.{self.casa_decimal_pressao}f}}}}} = "
                        rf"{lambda_flm_p.magnitude:.{self.casa_decimal_momento}f}"
                    )
                },
                {
    ...
    def combined_forces_l(self):
        ...
        vrd_x = self.shear_force_x_l()
        vrd_y = self.shear_force_y_l()
